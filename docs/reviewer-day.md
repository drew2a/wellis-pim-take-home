...
- **Order: people waiting come before data to clean.** The intakes waiting for a person sort above
  the review items, and inside each group the oldest is first; the "newest first" button flips the
  age inside each group and never moves the groups past one another. The two sources the queue
  combines are not equally urgent — a patient who submitted this morning is waiting for a decision,
  a consent gap from 2023 is not — and age alone cannot tell them apart: after a fresh import every
  review item carries the import moment as its `created_at`, so every one of the hundreds of open
  items is the same age and a new intake lands among them arbitrarily. A count in the rail proves
  an intake is in the view; it is not the same as a reviewer seeing it. Nothing else is sortable
  (a scope cut).
