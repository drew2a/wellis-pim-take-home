...
| 9 | Fields a tier-1 survivor gains from the loser | ADR-0006 requires them copied with per-field provenance in `changes`. Whether a later run may blank such a field again — the survivor's own raw row is empty, and canonical rows are rewritten from raw — was decided **after counting**, and the count is **zero**: over this export the 28 tier-1 pairs are identical on every one of the ten person fields, so no survivor gains anything and no later run can blank anything back. The copy is implemented as ADR-0006 requires and is tested on synthetic rows; the protection question does not arise here and is left unanswered rather than answered speculatively (YAGNI). A future export with a non-zero count reopens it. |
...
| 18 | Which patient a legacy row rewrites on a re-run | `patients` gains **`created_from_legacy_id`** (migration 0005, unique, null for a patient the new flow created), and the canonical patient load resolves an existing row through it rather than through `patient_legacy_ids`. The two answer different questions: the alias says whose records a legacy id belongs to **now** and a merge repoints it, while this column says which exported row built this canonical row and never changes. Without it the second run maps a merged-away row onto its survivor and overwrites the survivor with another row's values — 34 patients on this export, found by the double-run test. The alias table keeps its documented job: intakes and consent events are still loaded against it, so a row arriving later under a merged-away legacy id attaches to the survivor. |
| 19 | The weight-divergence detector finds nothing in this export | ADR-0005 predicts 3 kg row items; the count is **0**, because ADR-0009 item 2 was decided after it. All four diverging `kg` pairs are a signup weight of 6.5 to 7.8 kg against an intake weight of 5.7 to 8.3 kg (`rec2LxzDL6x93TyyD`/`INT-7194` at 0.744, `recdD2x9IV78bfJ1N`/`INT-7200` at 1.246, `recpwf2KUoxB269rI`/`INT-7198` at 1.153, `recSZ0iXEva15I79t`/`INT-7193` at 0.740), and the mapper now nulls both ends as implausible, so the detector has nothing to compare. Each of those patients already carries a plausibility item, and raising a divergence item as well would break `CLAUDE.md` §6: a value is never both auto-fixed and flagged. The detector stands, tested on synthetic rows at both tolerance boundaries, for Part B and for the next export; the report states the zero and why. The `lbs` vocabulary item is unaffected — those weights convert to plausible kilograms — and reports 49 of 68 intakes and 30 of 46 patients. |
| 20 | What "the same day" means for two intakes | Two intakes are a same-day pair when **any** plausible reading of their `submitted_at` values agrees: the reading under the separator convention or the reading the convention would give if it were wrong, and the date the row carries even where the mapper nulled it as impossible. This is the profile's definition (P-27) and it is what produces ADR-0006's five pairs; the convention alone produces four. Both widenings matter here: `INT-7500` (`04/12/2024`) pairs with `INT-7498` (`2024-04-12`) only because the readings meet, and `INT-7254`/`INT-7255` pair only because two rows that both say 2062-01-25 are still one patient submitting twice on one day. It is the date they share, not its plausibility, that makes them a pair; the shifted record is a separate item. |
| 21 | How a caller knows which rules fired | `EligibilityResult` gains **`matched`**, the rules that fired in the fixed rule order the reasons already follow. The history audit and the report count per rule, and re-deriving "the BMI rule fired" from the thresholds outside the engine would be a second implementation of the rules next to the one that decides — the engine owns the rules (ADR-0010). The change is additive: no outcome, reason string or threshold moves, and the five names are the two `REJECT_RULES` the ruleset's precedence already speaks plus the three that flag. |
| 22 | The consent items are counted over surviving patients | ADR-0005 counts 99 consent row items over the 2466 legacy rows (7 conflicts, 19 revoked while active, 73 without a record while active or paused). Over the 2438 surviving patients the number is **83**: the 7 and the 19 are unchanged, and 16 of the 73 are duplicate rows this run merges away. A merged row is not a second person to chase for consent, so the item goes with it. The report states both figures, as it does for the states themselves (item 13). |
...
- Integration test: after a merge, a second import changes no `patient_id` on any intake or consent
  event, rewrites no patient row, and changes no count in any table but `import_runs`.
...
- The count of item 9 is written into this ADR before the branch is reviewed: it is zero, and
  `candidates.test.ts` covers the copy on synthetic rows instead.
