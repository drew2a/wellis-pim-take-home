...
| # | Question | Decision |
| --- | --- | --- |
| 1 | Rule codes for values the mapper blanks | Every stored null whose raw value is non-empty has a normalisation record with `to_value` null and one of: `DATE_IMPOSSIBLE_TO_NULL` (dob after `--as-of` or giving an age above 100 at signup; `signup_date` or `submitted_at` after `--as-of`), `EMAIL_PLACEHOLDER_TO_NULL`, `EMAIL_INTERNAL_SPACE_TO_NULL`, `WEIGHT_UNIT_MISSING_TO_NULL`, `PHONE_UNPARSED_TO_NULL`, `IMPLAUSIBLE_TO_NULL`, `VOCAB_UNKNOWN` (closed vocabulary value not in the mapping table; canonical `unknown` where the enum has it, null otherwise). Records chain: a slash date in 2049 gets `DATE_ORDER_FROM_SEPARATOR` from the raw string to the read date, then `DATE_IMPOSSIBLE_TO_NULL` from the read date to null, so the ADR-0005 count of 638 holds. An empty raw value maps to null with no record: empty is empty. |
...
| 4 | Which actors are human | One exported set `SYSTEM_ACTORS` in `src/import/actors.ts`, today `legacy import` and `importer`. Part B adds the engine's actor for automatic transitions there and nowhere else. Any actor not in the set is human, and a field it names in `audit_entries.changes` is human-owned. |
...
| 8 | Unseen consent `action` or `type` | `consent_events.action` is an enum without `unknown` and the event cannot be stored without one. The raw row is stored (it always is), no canonical event is written, and one vocabulary item per unseen value is raised with the affected line numbers in its payload; the consent-state derivation of the detectors branch treats the patient's events as incomplete until the item is resolved. An unseen `type` is stored as exported (text column) with the same one item; the state is per type and a new type is not folded into `data_processing`. Zero rows in this export. |
