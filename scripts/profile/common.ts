...
const ORDER_CLASSES: readonly OrderClass[] = ['day-first', 'month-first', 'ambiguous', 'invalid', 'not-three-parts'];
...
export function dateAnalysis(values: readonly string[]): DateAnalysis {
  ...
  const rows: string[][] = [];
  const jsonShapes: Array<Record<string, unknown>> = [];
  const mixedShapes: string[] = [];
  const ordered = [...byShape.entries()].sort((a, b) => b[1].length - a[1].length || (a[0] < b[0] ? -1 : 1));
...
export function shapeCrossTab(
  ...
  const c = new Counter();
  shapes.forEach((s, i) => c.add(`${s}${KEY_SEP}${others[i] ?? ''}`));
  ...
  return table(caption, [`shape \\ ${otherLabel}`, ...colKeys.map((k) => (k === '' ? '(empty)' : plain(k))), 'total'], rows);
}
...
export interface NumericAnalysis {
  readonly parsed: ReadonlyArray<{readonly ok: boolean; readonly value: number; readonly separator: string}>;
  readonly notes: readonly string[];
  readonly tables: readonly Table[];
  readonly json: Record<string, unknown>;
}
 ...
 */
export function numericAnalysis(values: readonly string[], bandEdges: readonly number[], label: string): NumericAnalysis {
  ...
  return {
    ...
    notes: [
      `${nums.length} of ${values.length} values parse as a number; ${nonNumeric.entries().reduce((t, e) => t + e.count, 0)} ` +
        ...
        `${sepCounter.entries().map((e) => `${e.value} ${e.count}`).join(', ')}. Distribution of ${label}: ` +
...
const SEPARATORS: ReadonlyArray<readonly [string, RegExp]> = [
  [',', /,/u],
  [';', /;/u],
  [' en ', / en /u],
  [' and ', / and /u],
...
const EMPTY_LIKE = /^(geen|geen medicatie|geen bijzonderheden|none|no|nee|neen|n\/a|na|n\.?v\.?t\.?|nvt|nihil|niks|niet|-{1,3}|\.|x|0)$/u;
...
export function freeTextAnalysis(
  ...
  const emptyLike = foldedCounter.entries().filter((e) => e.value !== '' && EMPTY_LIKE.test(e.value));
  ...
  const tables: Table[] = [
    table(
      ...
      shown.map((e) => [code(e.value), String(e.count), e.value !== '' && EMPTY_LIKE.test(e.value) ? 'yes' : 'no']),
    ),
    table(
  ...
  const termJson: Array<Record<string, unknown>> = [];
  for (const t of terms) {
    ...
    termJson.push({term: t.label, patterns: t.patterns, rows: hits.length, distinctFoldedValues: distinct.size});
  }
  tables.push(table('Term counts (case-insensitive substring of the folded value)', ['term', 'rows', 'distinct folded values', 'examples'], termRows));
