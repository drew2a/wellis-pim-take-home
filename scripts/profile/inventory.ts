...
function prune(value: Json): Json | Pruned {
  if (Array.isArray(value)) {
    ...
    if (value.length > CAP && first !== undefined && typeof first === 'object') {
      ...
      return {
        __list: true,
        items: value.slice(0, CAP).map((v) => prune(v) as Json),
        total: value.length,
        remaining,
      };
    }
    if (value.length > 200)
      return { __list: true, items: value.slice(0, CAP), total: value.length, remaining: 0 };
...
if (source === null || typeof source !== 'object' || Array.isArray(source))
  throw new Error(`${PROFILE_JSON} is not an object`);
if (source.asOf !== asOf) {
  throw new Error(
    `${PROFILE_JSON} was generated as of ${JSON.stringify(source.asOf)}, not ${asOf}; run \`${reproduceCommand('profile', asOf)}\` first`,
  );
}
...
const data: Record<string, Json> = {
  ...source,
  columns: prune(source.columns ?? null) as Json,
  crossFile: prune(source.crossFile ?? null) as Json,
};
