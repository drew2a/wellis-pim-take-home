...
for (const [name, vals] of dateCols) for (const v of vals) { const d = readDateByShape(v); if (d && iso(d) > asOf) tail.add(name); }
table(['column', `rows dated after ${asOf}`], dateCols.map(([name]) => [name, tail.rows([name])[0]?.[1] ?? 0]));
